...
class CarrierDataUpdateCoordinator(DataUpdateCoordinator):
    ...
    async def _async_update_data(self):
        ...
        try:
                ...
                for system in self.systems:
                    try:
                        ...
                        if not self._is_unauthorized_error(server_error):
                            raise
                        should_escalate = self._record_unauthorized(
                            f"energy refresh for {system.profile.serial}"
                        )
            ...
            self.data_flush = True
            _LOGGER.debug(
                "unrecognized error so retying in default 30 minutes but refreshing all data then."
            )
    ...
    def _record_unauthorized(self, context: str) -> bool:
        ...
        if not self.unauthorized_outage_logged:
            _LOGGER.warning(
                "Carrier API returned unauthorized during %s; treating it as a transient blip and keeping last known data.",
                context,
            )
    ...
    async def async_perform_api_call(
        ...
        for attempt in range(2):
            try:
                ...
                if attempt == 0 and not should_escalate:
