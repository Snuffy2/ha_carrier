...
def _is_unauthorized_status(error: BaseException) -> bool:
    ...
    return code in (401, "401") or status in (401, "401")
...
def is_transient_transport_error(error: BaseException, _seen: set[int] | None = None) -> bool:
    ...
    seen = set() if _seen is None else _seen
    return _walk_transient_transport_error(error, seen)


def _walk_transient_transport_error(error: BaseException, seen: set[int]) -> bool:
    """Walk an exception chain looking for transient transport failures.

    Args:
        error: Exception to inspect.
        seen: Set of exception object ids already visited.

    Returns:
        bool: True when this error or a chained error is transient.
    """
    error_id = id(error)
    if error_id in seen:
        return False
    ...
    if isinstance(error, TRANSIENT_TRANSPORT_EXCEPTIONS):
        return True
    return any(
        _walk_transient_transport_error(chained_error, seen)
        for chained_error in _iter_exception_causes(error)
    )
